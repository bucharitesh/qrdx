/** biome-ignore-all lint/suspicious/noExplicitAny: fasle positive */
...
export async function getCommunityThemes(
  ...
  try {
    ...
    const communityThemeIds = themes.map((t: any) => t.id);
    const tagsRows =
    ...
    const mappedThemes: CommunityTheme[] = themes.map((row: any) => ({
      id: row.id,
      themeId: row.themeId,
      name: row.themeName,
